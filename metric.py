...
def batch_compute_metrics(batch_true_tags, batch_pred_tags, target_tags_no_schema):
    whole_res = dict()
    METRICS = ['strict', 'ent_type', 'partial', 'exact']
    SUB_METRICS = ['correct', 'incorrect', 'partial', 'missed', 'spurious', 'actual', 'possible']
    for metrics in METRICS:
        whole_res[metrics] = dict()
        for sub_metrics in SUB_METRICS:
            whole_res[metrics][sub_metrics] = 0

    batch_true_named_entities = [collect_named_entities(true_tags) for true_tags in batch_true_tags]
    batch_pred_named_entities = [collect_named_entities(pred_tags) for pred_tags in batch_pred_tags]

    for true_named_entities, pred_named_entities in zip(batch_true_named_entities, batch_pred_named_entities):
        res, _ = compute_metrics(true_named_entities, pred_named_entities, target_tags_no_schema)
        for metrics in METRICS:
            for sub_metrics in SUB_METRICS:
                whole_res[metrics][sub_metrics] += res[metrics][sub_metrics]
