...
class CRF(nn.Module):
    ...
    def neg_log_likelihood(
        ...
        # shape: (batch_size,)
        llh = (numerator - denominator) * (-1) # multiply -1 means negative log likelihood
    ...
    def forward(
        ...
        return self.neg_log_likelihood(emissions, tags, mask, reduction)
...
#     emission = torch.randn(2, 5, 5)
#     mask = torch.ByteTensor([[1, 1, 1, 0, 0], [1, 1, 1, 1, 0]])
#     tags = torch.tensor([[1, 2, 0, 0, 0], [2, 3, 0, 0, 0]], dtype=torch.long)
#     crf = CRF(5, True)
#     print(crf.decode(emission, mask))